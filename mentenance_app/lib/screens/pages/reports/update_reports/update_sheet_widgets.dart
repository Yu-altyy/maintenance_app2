        ...
        DropdownButtonFormField<String>(
          value:
              items.contains(value)
                  ? value
                  : null, // ✅ تأكد أن القيمة ضمن القائمة
          items:
              items
                  .map(
...
class _SavePrintButtonState extends State<SavePrintButton> {
  ...
  Future<void> _handleSaveAndPrint() async {
    ...
    try {
      ...
      if (reportId == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('❌ لا يوجد تقرير محدد للتعديل')),
        );
        return;
      }

      // 🟢 تحديث التقرير من السيرفر
        ...
        pw.MultiPage(
          ...
          build:
              (context) => [
                ...
                pw.Center(
                  child: pw.Text(
                    'تم إعداد التقرير والفاتورة تلقائيًا بواسطة نظام إدارة الصيانة',
                    style: pw.TextStyle(
                      font: arabicFont,
                      fontSize: 12,
      ...
      await Printing.layoutPdf(
        ...
        name: 'تقرير_وفاتورة_${data['REPORT_ID']}',
      );

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ تم تعديل وطباعة التقرير والفاتورة بنجاح'),
          backgroundColor: Colors.green,
        ),
      );
    } catch (e) {
      print('❌ خطأ أثناء تحديث التقرير أو الطباعة: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('❌ فشل في تعديل التقرير: $e'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
